...
<body>
	...
	<div class="sticky">
		...
		<nav class="navbar navbar-expand-lg navbar-dark" style="background-color:#006400">
		  <a class="navbar-brand" href="index.php">CCAcademy</a>
		</nav>
			...
			<div class="col-md-6 mx-auto shadow-lg p-3 mb-5 bg-white rounded lgndiv">
				...
				<form method="post">
				  ...
				  <div class="form-group">
					...
					<input type="number" class="form-control" name="contact" pattern="[0-9]{10}" MaxLength="11" required title="Kindly Enter 11 Digit Number" />
				  </div>
				  <div class="form-group">
				  ...
				  <input type="password" class="form-control" name="password" required pattern="^(?=.*[a-z])(?=.*[A-Z])(?=.*\d)(?=.*[!@#$%^&*])[A-Za-z\d!@#$%^&*]{12,}$" title="Password must be at least 12 characters long and include at least one uppercase letter, one lowercase letter, one number, and one special character (!@#$%^&*)."/>
				  </div> 
				  <button type="submit" class="btn btn-info" name="submit" style="background-color: darkgreen;" >Submit</button>
				</form>
			</div>
		</div>
